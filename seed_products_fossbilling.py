...
with open('d:\\Server\\www\\seed_products.sql', 'w') as f:
    f.write(sql)
...
sftp = ssh.open_sftp()
sftp.put('d:\\Server\\www\\seed_products.sql', '/tmp/seed_products.sql')
sftp.close()

stdin, stdout, stderr = ssh.exec_command('mysql -u fossbilling -pfossbilling_pass < /tmp/seed_products.sql')
