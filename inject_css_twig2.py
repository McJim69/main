...
# First, let's remove any previous injected premium CSS blocks in case we run this multiple times.
html = re.sub(r'<!-- Custom Integration Styling -->.*?</style>', '', html, flags=re.DOTALL)
html = re.sub(r'<style>\s*/\* PREMIUM MODERN OVERRIDES \*/.*?</style>', '', html, flags=re.DOTALL)
html = re.sub(r'<link rel="preconnect" href="https://fonts.googleapis.com">.*?Outfit.*?rel="stylesheet">', '', html, flags=re.DOTALL)
...
premium_css = """
    <style>
        ...
        /* Fix for Card Inside Card (Nested borders) */
        ...
        .card .card:hover, .card.border-0:hover {
