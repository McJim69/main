...
class BlogManager {
  ...
  static getActivePostId() {
    ...
    // 2. Element dataset check: .blog-details[data-post-id]
    const detailsSec = document.querySelector('.blog-details');
  ...
  static loadPosts(postId) {
    const detailsContainer = document.querySelector('.blog-details');
    ...
    if (isDetailsPage) {
      ...
      return fetch(`ajax_posts_details.php?id=${id}&_=${timestamp}`, { cache: 'no-store' })
        ...
        .then(html => {
          ...
          const newDetails = doc.querySelector('.blog-details');
          const currentDetails = document.querySelector('.blog-details');
    ...
    // Mode 2: Targeted Single Post Card Refresh on Feed Page (blog.php)
    if (id) {
      const cardSelector = `.blog-card [data-post-id="${id}"], .blog-card [data-id="${id}"], .blog-card a[href*="id=${id}"]`;
      const targetCardEl = document.querySelector(cardSelector);
      const targetCard = targetCardEl ? targetCardEl.closest('.blog-card') : null;

      if (targetCard) {
        console.log(`[BlogManager] Instantly refreshing targeted card for Post ID: ${id}`);
        return fetch(`ajax_posts_details.php?id=${id}&_=${timestamp}`, { cache: 'no-store' })
          .then(res => {
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            return res.text();
          ...
          .then(html => {
            const parser = new DOMParser();
            const doc = parser.parseFromString(html, 'text/html');
            const newCard = doc.querySelector('.blog-card');
            if (newCard && targetCard) {
              targetCard.outerHTML = newCard.outerHTML;
            }
            BlogManager.reinitVenobox();
          })
          .catch(err => {
            console.error(`[BlogManager] Error refreshing card ${id}, falling back to full feed refresh:`, err);
            return BlogManager.refreshFullFeed();
          });
      }
    }

    // Mode 3: Full Feed Refresh Fallback (blog.php)
    return BlogManager.refreshFullFeed();
  }
  ...
  static refreshFullFeed() {
    ...
    return fetch(`ajax_posts.php?_=${timestamp}`, { cache: 'no-store' })
      ...
      .then(html => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
        const newBlog = doc.querySelector('.blog');
        const currentBlog = document.querySelector('.blog');
